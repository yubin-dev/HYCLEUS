...
Bu paketin sınadığı beş tasarım kararı:
  1. Değişmemiş dosya GERİ YAZILMIYOR (gereksiz nonce/şifreleme yok)
  2. Aynı belge AYNI SÜREÇTE iki kez açılınca İKİNCİ KOPYA ÜRETİLMİYOR
  3. Geri yazma yarıda kesilirse orijinal `.hcl` BOZULMUYOR
  4. Geçici kopya `shred_file()` ile siliniyor — düz unlink değil
  5. Aynı belge BAŞKA bir uygulama örneğinde açıksa yalnızca BİRİ kazanıyor
     (bölüm 10) ve o örnek çökerse kilit açılışta SAHİPSİZ tanınıp temizleniyor
"""
...
def test_checkout_writes_the_plaintext_to_safezone(db, reg, hcl, key) -> None:
...
def test_the_temp_name_does_not_leak_the_document_name(db, reg, hcl, key) -> None:
    """
...
def test_the_extension_is_preserved(db, reg, hcl, key) -> None:
    """Varsayılan uygulamayı seçen şey uzantı — kaybolursa dosya açılmaz."""
    entry = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    assert entry.safe_path.suffix == ".txt"


def test_the_baseline_is_the_current_content(db, reg, hcl, key) -> None:
    entry = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    assert entry.baseline_sha256 == hashlib.sha256(_ILK).hexdigest()
    assert has_changed(entry) is False


def test_a_wrong_key_does_not_create_a_safezone_file(db, reg, hcl) -> None:
    """Çözülemeyen dosya için SafeZone'da artık kalmamalı."""
    with pytest.raises(CheckoutError):
        check_out(reg, db=db, file_id=1, hcl_path=hcl, key=generate_key())
...
def test_opening_twice_reuses_the_same_copy(db, reg, hcl, key) -> None:
    """
    ASIL TEST. İki kopya olsaydı kullanıcı ikisini de düzenler, son geri
    yazan diğerinin işini silerdi.
    """
    ilk = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    ikinci = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
...
def test_reopening_does_not_discard_unsaved_edits(db, reg, hcl, key) -> None:
    """İkinci açma, düzenlenmiş kopyanın üzerine ORİJİNALİ yazmamalı."""
    entry = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    _duzenle(entry)

    yeniden = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    assert yeniden.safe_path.read_bytes() == _YENI


def test_two_different_files_get_two_copies(db, reg, tmp_path, key) -> None:
    a = tmp_path / "a.txt"
    a.write_bytes(b"AAA")
    b = tmp_path / "b.txt"
    b.write_bytes(b"BBB")
    ...
    e1 = check_out(reg, db=db, file_id=1, hcl_path=ha, key=key, aad_hwid=_HWID)
    e2 = check_out(reg, db=db, file_id=2, hcl_path=hb, key=key, aad_hwid=_HWID)
    assert e1.safe_path != e2.safe_path
    assert len(reg) == 2


def test_a_vanished_copy_is_re_decrypted(db, reg, hcl, key) -> None:
    """
...
def test_an_edit_is_detected(db, reg, hcl, key) -> None:
    entry = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    _duzenle(entry)
...
def test_a_touched_file_is_not_a_change(db, reg, hcl, key) -> None:
    entry = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    st = entry.safe_path.stat()
...
def test_a_deleted_copy_is_not_a_change(db, reg, hcl, key) -> None:
...
def test_settling_waits_for_writes_to_stop(db, reg, hcl, key) -> None:
    """
...
def test_the_full_cycle_re_encrypts_the_edit(db, reg, hcl, key) -> None:
    """ANA AKIŞ: aç → değiştir → kapat → `.hcl` yeni içeriği taşıyor."""
    entry = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    _duzenle(entry)

    sonuc = check_in(reg, 1, key, db=db, user_id=_USER, hwid=_HWID)
...
def test_an_unchanged_file_is_not_re_encrypted(db, reg, hcl, key) -> None:
    """
    ...
    check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
...
def test_the_nonce_changes_on_every_rewrite(db, reg, hcl, key) -> None:
    """
...
def test_the_baseline_moves_after_a_writeback(db, reg, hcl, key) -> None:
...
def test_the_original_filename_survives_the_rewrite(db, reg, hcl, key) -> None:
    """
    ...
    entry = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    _duzenle(entry)
    check_in(reg, 1, key, db=db, user_id=_USER, hwid=_HWID)
...
def test_the_rewritten_file_still_verifies(db, reg, hcl, key) -> None:
...
def test_an_interrupted_rewrite_leaves_no_temp_file(db, reg, hcl, key, monkeypatch) -> None:
    entry = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    _duzenle(entry)
...
def test_the_edit_survives_a_failed_rewrite(db, reg, hcl, key, monkeypatch) -> None:
    """
...
def test_a_batch_checkin_continues_after_one_failure(db, reg, tmp_path, key, monkeypatch) -> None:
    """
    ...
    girdiler = [
        check_out(reg, db=db, file_id=i, hcl_path=p, key=key, aad_hwid=_HWID)
    ...
    sonuclar = check_in_all(reg, key, db=db, user_id=_USER, hwid=_HWID)
...
def test_the_temp_copy_is_shredded_on_checkin(db, reg, hcl, key) -> None:
    entry = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    yol = entry.safe_path
    _duzenle(entry)
...
def test_shredding_overwrites_before_unlinking(db, reg, hcl, key, monkeypatch) -> None:
    """
...
def test_discard_shreds_without_writing_back(db, reg, hcl, key) -> None:
    ...
    assert discard(reg, 1, db=db) is True
    assert not yol.exists()
    assert hcl.read_bytes() == onceki
    assert len(reg) == 0


def test_a_failed_shred_does_not_raise(db, reg, hcl, key, monkeypatch) -> None:
    """
...
def test_safezone_is_empty_after_a_full_cycle(db, reg, hcl, key) -> None:
    entry = check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    _duzenle(entry)
    check_in(reg, 1, key, db=db, user_id=_USER, hwid=_HWID)
    assert safezone.list_leftovers() == []
...
def test_lookup_by_safe_path(db, reg, hcl, key) -> None:
...
def test_checking_in_an_unknown_file_raises(db, reg, key) -> None:
    with pytest.raises(CheckoutError, match="Açık kayıt yok"):
        check_in(reg, 99, key, db=db, user_id=_USER)


def test_stale_files_are_reported(db, reg, hcl, key) -> None:
...
def test_an_unchanged_checkin_leaves_the_row_alone(db, reg, hcl, key) -> None:
    ...
    check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid=_HWID)
    apply_checkin(db, check_in(reg, 1, key, db=db, user_id=_USER), user_id=_USER)
...
def test_a_foreign_hwid_blocks_the_checkout(db, reg, hcl, key) -> None:
    """Başka cihazda şifrelenmiş dosya açılamamalı — GCM AAD kontrolü."""
    with pytest.raises(CheckoutError):
        check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key, aad_hwid="BASKA-HWID")


def test_the_rewrite_rebinds_to_the_editing_device(db, reg, hcl, key) -> None:
    """
...
# ══════════════════════════════════════════════════════════════════════════════
# 10. Kilit — aynı dosya BAŞKA bir uygulama örneğinde açıksa
# ══════════════════════════════════════════════════════════════════════════════
#
# Yukarıdaki bölüm 2 ("aynı belge iki kez açılırsa") TEK bir `reg`/süreç
# içindi. Burada İKİ AYRI `CheckoutRegistry` — iki AYRI uygulama örneğinin
# bellek içi kaydı — aynı `db`'yi (aynı disk, aynı `file_locks` tablosu)
# paylaşıyor; ayrım yalnızca `session_id`. Bu, gerçek iki süreci başlatmadan
# "iki örnek" senaryosunu ölçülebilir kılıyor.

#: Bu makinede/PID uzayında GERÇEKTEN çalışan bir sürece denk gelmesi
#: pratikte imkânsız bir sayı — "ölü" süreci simüle etmek için.
_OLU_PID = 2**30


def test_a_second_instance_cannot_open_a_file_the_first_has_open(db, hcl, key) -> None:
    """ASIL TEST — iki eşzamanlı 'aç' girişiminden yalnızca biri kazanır."""
    ornek_a = CheckoutRegistry()
    ornek_b = CheckoutRegistry()

    entry_a = check_out(
        ornek_a, db=db, file_id=1, hcl_path=hcl, key=key,
        aad_hwid=_HWID, user_id=_USER, session_id="ornek-A",
    )

    with pytest.raises(FileLockedError, match="başka bir uygulama örneğinde"):
        check_out(
            ornek_b, db=db, file_id=1, hcl_path=hcl, key=key,
            aad_hwid=_HWID, user_id=99, session_id="ornek-B",
        )

    # B kaybetti: B'nin SafeZone'unda hiçbir şey üretilmedi, A'nınki duruyor.
    assert len(ornek_b) == 0
    assert len(ornek_a) == 1
    assert entry_a.safe_path.is_file()


def test_the_same_instance_can_reopen_its_own_lock(db, hcl, key) -> None:
    """Aynı session_id kendi kilidine takılmamalı — bölüm 2'nin DB karşılığı."""
    reg = CheckoutRegistry()
    check_out(reg, db=db, file_id=1, hcl_path=hcl, key=key,
              aad_hwid=_HWID, session_id="ayni-ornek")
    # Kayıt defterinden düşse bile (ör. yeniden başlatma DEĞİL, aynı süreç
...
def test_closing_releases_the_lock_for_the_next_instance(db, hcl, key) -> None:
    """A kapatınca B artık normal şekilde açabilmeli."""
    ornek_a = CheckoutRegistry()
    ornek_b = CheckoutRegistry()

    check_out(ornek_a, db=db, file_id=1, hcl_path=hcl, key=key,
              aad_hwid=_HWID, session_id="ornek-A")
    check_in(ornek_a, 1, key, db=db, user_id=_USER, hwid=_HWID,
             session_id="ornek-A")

    entry_b = check_out(ornek_b, db=db, file_id=1, hcl_path=hcl, key=key,
                        aad_hwid=_HWID, session_id="ornek-B")
    assert entry_b.exists()


def test_discarding_releases_the_lock_for_the_next_instance(db, hcl, key) -> None:
    ornek_a = CheckoutRegistry()
    ornek_b = CheckoutRegistry()

    check_out(ornek_a, db=db, file_id=1, hcl_path=hcl, key=key,
              aad_hwid=_HWID, session_id="ornek-A")
    discard(ornek_a, 1, db=db, session_id="ornek-A")

    check_out(ornek_b, db=db, file_id=1, hcl_path=hcl, key=key,
              aad_hwid=_HWID, session_id="ornek-B")  # patlamamalı


def test_an_intermediate_writeback_keeps_the_lock(db, hcl, key) -> None:
    """`shred=False` (autosave) belgeyi AÇIK bırakıyor — kilit de KORUNMALI."""
    ornek_a = CheckoutRegistry()
    ornek_b = CheckoutRegistry()

    entry = check_out(ornek_a, db=db, file_id=1, hcl_path=hcl, key=key,
                      aad_hwid=_HWID, session_id="ornek-A")
    _duzenle(entry)
    check_in(ornek_a, 1, key, db=db, user_id=_USER, hwid=_HWID,
             reason="autosave", shred=False, session_id="ornek-A")
...
# ── Çökme + açılış kurtarması ────────────────────────────────────────────────


def test_a_lock_from_a_dead_process_is_released_on_startup(db, hcl, key) -> None:
    """
    ASIL TEST — sahipsiz kalan kilit dosyayı KALICI kilitli bırakmamalı.

    Önceki oturum dosyayı açıkken çökmüş gibi (kayıt defteri de kayboldu —
    yeni süreç kendi BOŞ registry'siyle başlıyor) bir kilit satırı elle
    kuruluyor, `pid` GERÇEKTEN yaşamayan bir sayı.
    """
    acquire_lock(db, file_id=1, user_id=_USER, session_id="cokmus-oturum",
                 pid=_OLU_PID)

    rapor = release_stale_locks(db)
    assert rapor.released == 1
    assert rapor.released_ids == [(1, socket.gethostname())]

    # Devralma: bir SONRAKİ süreç şimdi normal şekilde açabiliyor.
    yeni_ornek = CheckoutRegistry()
    entry = check_out(yeni_ornek, db=db, file_id=1, hcl_path=hcl, key=key,
                      aad_hwid=_HWID, session_id="yeni-oturum")
    assert entry.exists()


def test_a_lock_from_a_live_process_survives_the_sweep(db, hcl, key) -> None:
    """Hâlâ ÇALIŞAN bir sürecin kilidi açılış süpürmesinde YANLIŞLIKLA silinmemeli."""
    acquire_lock(db, file_id=1, user_id=_USER, session_id="canli-oturum",
                 pid=os.getpid())  # test sürecinin kendi PID'i — GERÇEKTEN canlı
    ...
    yeni_ornek = CheckoutRegistry()
    with pytest.raises(FileLockedError):
        check_out(yeni_ornek, db=db, file_id=1, hcl_path=hcl, key=key,
                  aad_hwid=_HWID, session_id="baska-oturum")


def test_a_lock_from_a_different_machine_is_never_swept(db, hcl, key) -> None:
    """
    Başka bir makineye ait bir kilidin canlılığı BURADAN doğrulanamaz —
    modül docstring'indeki "PID canlılığı": yanıtlanamayan soruya "ölü"
    demek YANLIŞ tarafta hata yapmak olurdu, bu yüzden DOKUNULMAZ.
    """
    acquire_lock(db, file_id=1, user_id=_USER, session_id="baska-makine",
                 pid=_OLU_PID, hostname="BASKA-MAKINE")
    ...
    with pytest.raises(FileLockedError):
        check_out(yeni_ornek, db=db, file_id=1, hcl_path=hcl, key=key,
                  aad_hwid=_HWID, session_id="ucuncu-oturum")


def test_sweep_with_nothing_stale_is_a_clean_report(db) -> None:
    rapor = release_stale_locks(db)
    assert rapor.released == 0
